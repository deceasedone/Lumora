...
export function DotGridProgress({
  ...
  const dotSize = size === "lg" ? "w-2.5 h-2.5" : size === "sm" ? "w-1.5 h-1.5" : "w-[3px] h-[3px]"
  const gap = size === "lg" ? "gap-1.5" : size === "sm" ? "gap-1" : "gap-[2px]"

  return (
    <div
      className={cn(
        "rounded-[var(--radius)] p-5 bg-[var(--card)] border border-[var(--border)] flex flex-col justify-between",
        size === "lg" ? "aspect-[4/3]" : size === "sm" ? "aspect-square" : "",
        className,
      )}
    >
      <div>
      ...
      <div className={cn("flex flex-wrap content-start", gap)}>
        {dots.map((_, i) => (
          <div
            key={i}
